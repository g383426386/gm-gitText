...
- (void)requestPermission:(WQPermission)permission
    ...
    WeakSelf
    [[[UIAlertView alloc]initWithTitle:title message:description delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"设置", nil] showWithCompletionHandler:^(NSInteger buttonIndex) {
        StrongSelf
        if (buttonIndex == 1) {
            ...
            NSError *error = [NSError errorWithDomain:WQErrorDomain
                                             ...
                                             userInfo:@{NSLocalizedDescriptionKey :@"禁止开启权限"}];
            result(NO,error);
...
- (void)requestPermission:(WQPermission)permission
              ...
              description:(NSString *)description
            requestResult:(WQRequestResult)result
          callbackSetting:(void(^)())settingActionBlock{
